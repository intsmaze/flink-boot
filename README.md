...
2. 也可以选择不打赏博主，公开版仅提供了Flink与以上Spring组件集成的所有代码，仅提供Flink与Spring基础集成的配置文件，其他组件的配置文件未提供，一般来说，自行研究框架3小时即可搞定。
3. 46.8元的打赏不是为了挣钱，只是为了让博主看到这个项目的价值有继续无私迭代的动力，为后续打造一级的开源项目做贡献（PS：收钱源自开发这套脚手架导致自己的就业面变窄）。
